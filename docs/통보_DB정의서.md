...
① 서버(SERVER_ID) 목록별로 워터마크 조회
   ...
   → (워터마크 유무와 무관) AGGREGATE_FROM이 당일 00:00:00 이전이면 당일 00:00:00으로 clamp
     (에러 0건 폴링이 반복되어 워터마크가 며칠째 전진하지 않은 경우, 자정 이전 미통보분은 집계 제외됨. 2026-08-18 추가)
   → clamp 후에도 AGGREGATE_FROM > AGGREGATE_TO(워터마크 역전)면 그 서버만 skip, log.warn만 남기고 다음 서버로 진행
        |
        ↓
② TB_ANALYZE_RESULT 집계 (서버별, AGGREGATE_FROM ~ 이번 스케쥴 기준시각)
   ...
   - 정상/경고/에러 건수 집계
        |
        ↓
③ 에러가 0건이면 → 그 서버는 통보 대상 아님, 스킵 (TB_NOTIFY_HISTORY 행도 생성 안함, 경고만 있어도 스킵)
   에러가 1건 이상이면 → ④로 진행
        |
        ↓
...
--   - 에러가 0건인 서버는 통보 대상이 아니므로 이 테이블에 행이 생성되지 않음(경고만 있어도 미생성)
...
CREATE TABLE TB_NOTIFY_HISTORY (
    ...
    -- ERROR_COUNT > 0 인 경우에만 이 행이 생성됨(WARNING_COUNT는 통보 판단에 관여하지 않음)
