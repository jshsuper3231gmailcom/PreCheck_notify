...
구현 완료(2026-07-20 검증, `gradlew test --rerun-tasks` 49개 테스트 전부 통과 — bind body 인코딩 + bind/submit ack 읽기 반영). HTTP 포트 없는 헤드리스 `@Scheduled` 배치 서버. 코드 동작은 `FLOW.md`, 설계 근거는 `docs/` 참조.
...
**핵심 결정:**
- 스케줄당 TCP 커넥션 1개 공유: `bind(01)` 1회 → 수신자마다 `submit(03)` 1회
- bind/submit 둘 다 `SmsAckPacket`(7바이트) 응답 수신. bind는 `result=="00"` 검증(실패 시 런 전체 FAIL), submit은 값 검증 없이 응답 도착 여부만 확인(응답 못 받으면 그 수신자만 실패) — `docs/통보_TR연동정의서.md` 2/5절
